...
%% Uses IGRF-14 and Gerhardt's Differential Flatley-Henretty Hysteresis 

% NOTE:
% This is the most accurate hysteresis simulation developed for my project
% so far. It can be improved by implementing dynamic environmental torques
% and orbital dynamics.

% This LGVI GASRATS case uses m_bar = 0.09 A*m^2.

% H and Hdot is frozen during the RK4 hysteresis integration. This can be
% improved at a future point.

% This simulator uses the differential Flatley-Henretty model with open
% circuit hysteresis parameters.
...
% Open circuit B_s: 0.3 T
% Open circuit B_r: 6.0618*1e-4 T
% Open circuit H_c: 0.3381 A/m
% Permanent magnet: 0.1 A*m^2
% Timestep: 0.05 s
% Field model: IGRF
% Altitude: 400 km
...
for k = 1:N
    ...
    % Hysteresis invariant region check from Park
    % xi1 = H1 - Hr * tan(pi * Brod(1) / (2 * Bs));
    % xi2 = H2 - Hr * tan(pi * Brod(2) / (2 * Bs));
    ...
    M_k = mag_term * cross(b3, H_body) + ...
          (n_rods_per_axis * Brod(1) * Vh) * cross(b1, H_body) + ...
          (n_rods_per_axis * Brod(2) * Vh) * cross(b2, H_body);
    ...
    M_kp1 = mag_term * cross(b3, H_body_next) + ...
            (n_rods_per_axis * Brod_next(1) * Vh) * cross(b1, H_body_next) + ...
            (n_rods_per_axis * Brod_next(2) * Vh) * cross(b2, H_body_next);
...
%%%%%%%%%%%%%%%%%%%%%%%%%% SAVE BLOCK %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
saveDir = pwd;
filename = 'AttitudeSim006_CF_ResultsV9_RK4_2RodsWeakMag0.09.mat';
fullPath = fullfile(saveDir, filename);
% Downsample saved data to reduce .mat file size
save_stride = 10;       % one pt saved every 10 sec
...
if sat_guard_count > 0
    fprintf('  (Brod was clamped to ±Bs on %d occasions)\n', sat_guard_count);
end
%%%%%%%%%%%%%%%%%%%%%%% PLOTTING %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
N_plot = length(t);
days = t / 86400;
...
exportgraphics(fig1,'006_CF_Pointing_Error_V9_RK4_2RodsWeakMag0.09.png','ContentType','vector','Resolution',600)
%% (b) Angular Velocity
...
set(gca,'FontSize',fontAxis,'LineWidth',1.0)
exportgraphics(fig2,'006_CF_Angular_Velocity_V9_RK4_2RodsWeakMag0.09.png','ContentType','vector','Resolution',600)
%% (c) Hysteresis Rod 1
fig3 = figure('Color','w', ...
              'Units','normalized', ...
              'Position',[0.05 0.08 0.9 0.85], ...
...
set(gca,'FontSize',fontAxis,'LineWidth',1.0)
exportgraphics(fig3,'006_CF_Rod1_V9_RK4_2RodsWeakMag0.09.png','ContentType','vector','Resolution',600)
%% (d) Hysteresis Rod 2
fig4 = figure('Color','w', ...
              'Units','normalized', ...
              'Position',[0.05 0.08 0.9 0.85], ...
...
ylabel('Induced Flux Density B [T]','FontSize',fontLabel)
set(gca,'FontSize',fontAxis,'LineWidth',1.0)
exportgraphics(fig4,'006_CF_Rod2_V9_RK4_2RodsWeakMag0.09.png','ContentType','vector','Resolution',600)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
